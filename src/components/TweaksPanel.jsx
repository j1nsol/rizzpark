/**
 * TweaksPanel — floating developer tweaks shell.
 * Listens for __activate_edit_mode / __deactivate_edit_mode postMessages
 * and announces availability via __edit_mode_available.
 */
...
export function TweakText({ label, value, placeholder, onChange }) {
  return (
    <TweakRow label={label}>
      <input
        className="twk-field" type="text" value={value}
...
export function TweakColor({ label, value, onChange }) {
  return (
    <div className="twk-row twk-row-h">
      <div className="twk-lbl"><span>{label}</span></div>
      <input
        type="color" className="twk-swatch" value={value}
        onChange={(e) => onChange(e.target.value)}
      />
    </div>
  );
